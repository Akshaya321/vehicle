...
$a=$_POST["Name"];
 $b=$_POST["DOB"];
 $c=$_POST["Gender"];
 $Photo=time().htmlspecialchars($_FILES['images']['name']);
  $path="D:/xampp/htdocs/vidhya/jobboard/reg/photo";

 move_uploaded_file($_FILES['images']['tmp_name'],"$path/".$Photo);
 $i=$_POST["Phoneno"];
...
//  move_uploaded_file($_FILES['photo']['tm_name'],$photo);
 ...
 echo $update="update `userrregistration` set `Name`='$a',`DOB`='$b',`Gender`='$c',`Phoneno`='$i',`Photo`='$Photo' where Userid=$id";
...
?>
         ...
         <td><input type="text" id="item" name="Accno"  value="<?php echo $rest['Accno'];?>"   style="height:25px;" /><br /> </td></tr>
          ...
          <tr><td><label for="item">Photo</label></td>
         <td> <img src="/vidhya/jobboard/reg/photo/<?php echo $rest['Photo'] ;?>"   style="height:25px;" /><br /> </td>
